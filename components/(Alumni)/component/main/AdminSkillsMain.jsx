...
const AdminSkillsMain = () => {
  ...
  const GetSkills = async (page) => {
    try {
      setLoading(true);
      const res = await Models.masters.skillList(page);
      setState({
        skillList: res?.results,
        currentPage: page,
        next: res?.next,
  ...
  const handleDepartmentSubmit = (e) => {
    ...
    if (isEditing) {
      ...
      axios
        .post(`${BaseURL}/update_skill/${formData.id}/`, formData, {
          headers: {
      ...
      axios
        .post(`${BaseURL}/create_skill/`, formData, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
  ...
  const editDepartment = (department) => () => {
    ...
    setFormData({
      id: department.skill_id,
      skill: department.skill,
        ...
        <div className="content">
          <div className="d-flex justify-content-between py-4 mb-4 ">
            <div className="section-title">
              <h4 className="mb-0 font-20 text-dark">Skills </h4>
            </div>
            ...
            {loading ? (
              ...
              <>
                <table className="rbt-table table table-borderless">
                  ...
                  <tbody>
                    {state.skillList?.map((item, index) => (
                      <tr key={index}>
                        <th>
                          <p className="b3 mb--5">{item.skill}</p>
                        </th>
                        <th>
...
export default AdminSkillsMain;
