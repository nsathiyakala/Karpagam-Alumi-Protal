...
const AdminIndustryTypeMain = () => {
  ...
  const GetRole = async (page) => {
    try {
      setLoading(true);
      const res = await Models.masters.bussinessTypeList(page);
      setState({
        bussinessTypeList: res?.results,
        currentPage: page,
        next: res?.next,
  ...
  const handleDepartmentSubmit = (e) => {
    ...
    if (isEditing) {
      console.log("Updating Industry Type:", formData);
      // Perform update logic here

      axios
        .post(`${BaseURL}/update_industry_type/${formData.id}/`, formData, {
          headers: {
      ...
      console.log("Creating Industry Type:", formData);
      // Perform create logic here

      axios
        .post(`${BaseURL}/create_industry_type/`, formData, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
  ...
  const editDepartment = (department) => () => {
    ...
    setFormData({
      id: department.id,
      type_name: department.type_name,
        ...
        <div className="content">
          <div className="d-flex justify-content-between py-4 mb-4 ">
            <div className="section-title">
              <h4 className="mb-0 font-20 text-dark">Business Type </h4>
            </div>
          ...
          <div className="rbt-dashboard-table table-responsive mobile-table-750 mt--30">
            ...
            ) : state.bussinessTypeList?.length > 0 ? (
              <>
                <table className="rbt-table table table-borderless">
                  ...
                  <tbody>
                    {state.bussinessTypeList?.map((item, index) => (
                      <tr key={index}>
                        <th>
                          <p className="b3 mb--5">{item.type_name}</p>
                        </th>
                        <th>
                ...
                {state.bussinessTypeList?.length > 9 && (
                  <div>
                    <div
                      className="mb-20 "
                      style={{
...
export default AdminIndustryTypeMain;
