...
const AdminLocationsMain = () => {
  ...
  const GetLocation = async (page) => {
    try {
      setLoading(true);
      const res = await Models.masters.locationList(page);
      setState({
  ...
  const success = (successMsg) => {
    messageApi.open({
      type: "success",
      content: successMsg || "Success! Check your email for further Location.",
    });
  };
  ...
  const handleDepartmentSubmit = (e) => {
    ...
    if (isEditing) {
      ...
      axios
        .post(`${BaseURL}/update_location/${formData.id}/`, formData, {
          headers: {
      ...
      axios
        .post(`${BaseURL}/create_location/`, formData, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
        ...
        <div className="content">
          <div className="d-flex justify-content-between py-4 mb-4 ">
            <div className="section-title">
              <h4 className="mb-0 font-20 text-dark">Location </h4>
            </div>
            ...
            {loading ? (
              ...
              <>
                <table className="rbt-table table table-borderless">
                  ...
                  <tbody>
                    {state.locationList?.map((item, index) => (
                      <tr key={index}>
                        <th>
                          <p className="b3 mb--5">{item.location}</p>
                        </th>
...
export default AdminLocationsMain;
